    ...
    with engine.connect() as conn:
        ...
        for d in downld_data.all():
            ...
            if email not in user_stats:
                user_stats[email] = [None, 0,0,0, None] # [user name, downloads today, downloads for month, downloads for 3 months, active account]
        ...
        # create message
        summary = f"Summary (Define active ac as download docs within the last 5 DAYS):\n---------------\nActive Accounts: {len([1 for x in user_stats if user_stats[x][4]])}/{total_users}\nInactive Accounts:{len([1 for x in user_stats if user_stats[x][4]==False])}/{total_users}"
        active_template = "{1}    ({0}):\n    today : {2}\n    this month : {3}\n    past 3 months : {4}"
        ...
        # print(message)
        # send message
        bot.sendMessage(chat_id=config.TELE_CHAT_ID, texts=message)
